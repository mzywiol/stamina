...
package object migrations {
   ...
   */
  def from[T, V <: V1: VersionInfo]: ForwardMigrator[T, V] = new ForwardMigrator[T, V](Map(Version.numberFor[V] → identityMigration[T]))
}
...
package migrations {
   ...
   * `andBackFrom[V]` can be used to define migration from version
   * one generation newer than current `Migrator` version.
   ...
   * Note that `andBackFrom[V]` can only be called once and the last one
   * in call chain when defining `Migrator[R, V]`.
   *
   ...
   */
  class Migrator[R, V <: Version: VersionInfo] private[stamina] (val migrations: Map[Int, Migration[R]], backwardMigration: Option[Migration[R]] = None) {
  ...
  private[stamina] class ForwardMigrator[R, V <: Version: VersionInfo](override val migrations: Map[Int, Migration[R]]) extends Migrator[R, V](migrations) {
    override def canMigrate(fromVersion: Int): Boolean = migrations.contains(fromVersion)

    def to[NextV <: Version: VersionInfo](migration: Migration[R])(implicit isNextAfter: IsNextVersionAfter[NextV, V]): ForwardMigrator[R, NextV] = {
      new ForwardMigrator[R, NextV](
